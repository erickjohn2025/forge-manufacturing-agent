  ...
  return <div className="configuration-grid">
    ...
    <section className="config-card"><div className="config-title"><span><SlidersIcon /></span><div><h3>Autonomy policy</h3><p>Boundaries your agent will always respect.</p></div></div>
      <form className="policy-form" onSubmit={async (event) => {
        ...
        <label>Autonomous purchase limit<input type="number" min={0} step={1} value={limit} onChange={(event) => setLimit(Number(event.target.value))} /></label>
        <small>Purchases above this amount require an approver. Preview: {formatMoney(limit, config.currency)}</small>
        <label>Default safety stock<input type="number" min={0} step={1} value={safety} onChange={(event) => setSafety(Number(event.target.value))} /></label>
        ...
        <button className="primary-button" disabled={saving} type="submit">{saving ? "Saving…" : "Save policy"}</button>
      </form>
    </section>
